#!/usr/bin/env python3
"""Build single-disc-on-csr-v0.1.9: restore CSR D1 LOSIN2 (sets GM 0xa455)."""
...
def main() -> int:
    ...
    work = ROOT / "workspace/iso-extract/sd_v019_losin2_d1.bin"
    if not work.is_file():
    ...
    pack_id = "single-disc-on-csr-v0.1.9"
    ...
    layer = build_layer(
        ...
        description=(
            "Single-disc on CSR v0.1.9 — restore CSR D1 LOSIN2 "
            "(end-D1 sets GM 0xa455 for break; keep D2 LOST2/COS_BTM2)"
        ),
    )
    ...
    old = json.loads(
        (ROOT / "builder/single-disc-on-csr-v0.1.8/pack.json").read_text()
    )
    pack = {
        **{k: v for k, v in old.items() if k not in ("id", "version", "blurb", "betaNote")},
        ...
        "blurb": (
            ...
            "End-of-D1 LOSIN2 keeps CSR D1 (sets break GameMoment). "
            "D2 LOST2/COS_BTM2 kept. Endings auto-apply."
        ),
    ...
    for a in man["addons"]:
        if a["id"].startswith("single-disc-on-csr-v0.1.") and a["id"] != pack_id:
            # disable older main packs only (not movies/endings)
            ver = a["id"].rsplit("-v", 1)[-1]
            try:
    ...
    if pack_id in ids:
        ...
        idx = next(
            ...
            if a["id"] == "single-disc-on-csr-v0.1.8"
        )
    ...
    (ROOT / "mods/single-disc/VERSION").write_text("0.1.9\n")

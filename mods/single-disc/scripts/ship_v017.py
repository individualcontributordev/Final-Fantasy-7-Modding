#!/usr/bin/env python3
"""Build single-disc-on-csr-v0.1.7 layer + manifest from sd_v017 work bin."""
...
def main() -> int:
    ...
    work = ROOT / "workspace/iso-extract/sd_v017_cos_btm2_break.bin"
    if not work.is_file():
    ...
    pack_id = "single-disc-on-csr-v0.1.7"
    ...
    layer = build_layer(
        ...
        description=(
            "Single-disc on CSR v0.1.7 — COS_BTM2 disc-id IFUW force "
            "for break scene (plus 0.1.6 LOST2/Ask)"
        ),
    )
    ...
    old = json.loads(
        (ROOT / "builder/single-disc-on-csr-v0.1.6/pack.json").read_text()
    )
    pack = {
        **{k: v for k, v in old.items() if k not in ("id", "version", "blurb", "betaNote")},
        ...
        "blurb": (
            ...
            "Forces disc1-to-disc2 break (LOST2+COS_BTM2). "
    ...
    for a in man["addons"]:
        if a["id"] == "single-disc-on-csr-v0.1.6":
            a["enabled"] = False
    if pack_id in ids:
    ...
    (ROOT / "mods/single-disc/VERSION").write_text("0.1.7\n")

#!/usr/bin/env python3
"""Diff a combined single-disc work bin against pristine D1 into ic-layer-v1.
...
def main() -> int:
    ...
    ap.add_argument("--work", type=Path, required=True, help="Combined single-disc D1 .bin")
    ap.add_argument("--pristine", type=Path, required=True, help="Pristine D1 .bin")
    ap.add_argument(
        "--version",
        default=None,
        help="Pack version override (default: mods/single-disc/VERSION)",
    )
    ap.add_argument(
        "--out-dir",
        type=Path,
    ...
    pack_id = "single-disc-clean-v%s" % ver
    ...
    desc = (
        "Single-disc Clean D1: Makou Ask-for-disc removal + SNOVA/BATTLE.X LBA v3 "

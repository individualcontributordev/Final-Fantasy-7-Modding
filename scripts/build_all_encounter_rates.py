#!/usr/bin/env python3
"""Build Encounter 25%/50%/75% layers for every current base (Disc 1 by default).

Needs workspace/pristine/FINALFANTASY7_D1.bin and network (for CSR base layers).

  python scripts/build_all_encounter_rates.py
  python scripts/build_all_encounter_rates.py --discs 1 --rates 25,50,75

Then commit builder/ JSON and push.
"""
...
BUILD = _SCRIPTS / "build_encounter_on_base.py"
...
def main() -> int:
	ap = argparse.ArgumentParser(description="Build all Encounter rate × base packs.")
	...
	ap.add_argument("--version", default=None, help="Override ENCOUNTER_VERSION")
	args = ap.parse_args()

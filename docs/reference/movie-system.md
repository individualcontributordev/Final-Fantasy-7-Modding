...
| Post-final-battle ending sequence needs ENDING2E at its D3 absolute LBA (197242) | confirmed (corrected) | Two live DuckStation captures, 2026-08-30. First `dslogs.txt` (partial) showed `Setloc` to MSF 43:51:67 = LBA 197242 (ENDING2E's D3 start); a build with only ENDING01 aliased played a corrupted fragment of whatever D1 file occupied LBA 197242 instead. A second, full save-state→4-fights→ending capture confirmed **no** `Setloc` to ENDING01's LBA (163608) or ENDING3E's LBA (172631) ever fires — the only ending-related seek in this trigger path is the single jump straight to 197242. The earlier "restore all three" fix worked, but only the ENDING2E piece was load-bearing; `alias_d3_ending_lbas_on_d1.py` now aliases ENDING2E only. |

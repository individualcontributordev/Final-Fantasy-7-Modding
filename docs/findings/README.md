...
|------|------|---------|------------|
| 2026-08-27 | [fship12-md8-5-mid53-lba-collision](2026-08-27-fship12-md8-5-mid53-lba-collision.md) | 67->731 movie still missing after v0.1.6 (which fixed a different branch, 67->269); real site is MD8_5 `dir` Main `PMVIE id=53`; `MOVIE_ID.BIN` row 53's LBA points at `/MOVIE/OPENINGE.MOV`'s appended block instead of the intended NRCRLB content — confirmed broken in reference bin too, not a single-disc regression | confirmed |
| 2026-08-27 | [field-dat-vram-base-header-bug](2026-08-27-field-dat-vram-base-header-bug.md) | `write_field_dat()` rebuilt every FIELD/*.DAT's 28-byte VRAM pointer header with hardcoded `0x80000000` instead of the field's own `vbase`, corrupting 6 precision-patched fields (JUNAIR, WHITE2, BUGIN1A, NIVGATE, RCKTIN2, BLACKBGB splice) into black-screen hangs; fixed to reuse `fd.vbase`. Includes field DAT format + VRAM texture-cache research references for future debugging | confirmed |
| 2026-08-26 | [junair-single-disc-battle-return-freeze](2026-08-26-junair-single-disc-battle-return-freeze.md) | JUNAIR (field 384) freeze returning from battle on single-disc-on-csr, reproduces with/without movie-relocation patch (rules that patch out); encounter table + SCENE.BIN confirmed byte-identical D1 vs D2; `air0`/slot-3 `AKAO(0xF2)` script delta also disproven (gated by an unhit line trigger) — no JUNAIR.DAT content difference explains the freeze, so cause is outside this file (engine state / shared file / audio-CD layout) | unverified |
| 2026-08-25 | [movie-relocation-plan](2026-08-25-movie-relocation-plan.md) | Re-verified 08-24's 17-mismatch list against `slot_live` liveness: only 4 real live conflicts remain (LOSLAKE1/CANONON already shipped; JUNAIR/GELNICA + TRNAD_51/C_SCENE1+3+FF_DAIKU need repoint/table-growth); ~39 MB new content fits in ~81 MB free budget | confirmed |
...
| 2026-08-24 | [field-bin-pmvie-movie-mvief-handlers-located](2026-08-24-field-bin-pmvie-movie-mvief-handlers-located.md) | Real FIELD.BIN opcode dispatch table found at PTR_LAB_800e0228 (byte-indexed, read directly from field script); PMVIE/MOVIE/MVIEF handlers confirmed via string xrefs (0xF8/0xF9/0xFA); no CD-read call found yet, state machine only; PTR_PTR_800df11c ruled out as false lead (GTE/animation dispatch) | in-progress |
| 2026-08-24 | [canonon-hardcode-scus94163-negative-result](2026-08-24-canonon-hardcode-scus94163-negative-result.md) | Full-decompile grep of SCUS_941.63 (kernel exe) for CANONON hardcode: zero hits on movie name strings, id-47 branches, or any field-opcode dispatcher; MOVIE_ID.BIN reader is a generic bulk-load with no id indexing visible; re-scopes hypothesis to CANONON's own field script, not the kernel | negative result |
| 2026-08-24 | [akao-opcode-0xf2-is-canonon-cd-call-site](2026-08-24-akao-opcode-0xf2-is-canonon-cd-call-site.md) | Ghidra headless decompile of `FUN_800c46d0` (the confirmed `ra=0x800C47CC` call site) + byte-scan for its address in the `PTR_LAB_800e0228` opcode table found it at index `0xF2` = **AKAO**, not PMVIE/MOVIE/MVIEF (0xF8/F9/FA, already ruled out); AKAO reads literal CD-command operand bytes straight from the field's own compiled script (no MOVIE_ID.BIN lookup), explaining why patching MOVIE_ID.BIN doesn't affect CANONON | confirmed |
| 2026-08-24 | [canon-fields-akao-operand-addressing-modes](2026-08-24-canon-fields-akao-operand-addressing-modes.md) | Decompiled AKAO's operand helpers (`FUN_800bf908`/`FUN_800bee10`): they use a 16-mode selector nibble (mode 0 = literal, others = bank/global lookups), correcting the "always literal" assumption; enumerated all AKAO instructions in CANON_1 (2, both cmd=0x40) and CANON_2 (24, varied cmd bytes) via `scripts/field_dat.py` — none match the traced `a1=0xC` in literal mode, so the real instruction likely uses a non-zero selector nibble | partial / open |
| 2026-08-24 | [canonon-hardcode-clean-room-reverification](2026-08-24-canonon-hardcode-clean-room-reverification.md) | Clean-room re-derivation from raw bytes + Makou source only: PMVIE is a disc-agnostic 1-byte id, no LBA anywhere in opcode/script/exe; 3 independent sources agree on D2 id 47 / D3 id 25; supersedes prior "hardcoded LBA" claim pending live single-variable test | likely |
| 2026-08-24 | [csr-movie-reachability-scan](2026-08-24-csr-movie-reachability-scan.md) | CFG-based PMVIE/MOVIE reachability analyzer (validated vs 3 known CSR skips); found PMVIE id resolves via MOVIE_ID.BIN row order, not sorted MOVIE/ dir order; **mismatch table superseded 2026-08-25** — see movie-relocation-plan | confirmed (mismatch table superseded) |
| 2026-08-24 | [kernel-exe-all-discs-no-movie-hardcode](2026-08-24-kernel-exe-all-discs-no-movie-hardcode.md) | Decompiled kernel EXE for all 3 discs (SCUS_941.63/64/65); CD dispatcher byte-identical across discs, no literal id-47 constant, and the mode-0xb movie-streaming primitive (FUN_80033e74) has zero in-file callers on any disc — real caller lives in a third, still-unidentified module | negative result |
...
| 2026-08-09 | [fanfare-skip-015-gap-ceremony-still-plays](2026-08-09-fanfare-skip-015-gap-ceremony-still-plays.md) | v0.1.6 confirmed working | confirmed |

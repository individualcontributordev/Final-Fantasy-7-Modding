...
single-disc-on-csr. Only CSR+ stacks skip the movie pack.
...
- CSR base + single-disc: keep manip-critical FMV streams correct (copy D2/D3
  MOVIE files onto D1 + leave Play movie for those sites).
- CSR + CSR+ scene packs + single-disc: do not need those copies (scenes cut);
...
files that an earlier single-disc layer already injected.
...
|-------------------|----------|----------------|
| single-disc-on-csr-vX | Ask trims, non-manip field movie Set+Play trims (crawl/unblock), SNOVA+BATTLE.X | Always with CSR single-disc |
| single-disc-csr-manip-movies-vX | ISO inject of whitelist D2/D3 MOVIE only; no Play deletion for those sites | **Required** with CSR base single-disc; **omit** when CSR+ scene packs are used |

User stacks:

- CSR base, single disc: Base CSR + single-disc-on-csr + manip-movies (**both required**)
- CSR + CSR+ scenes, single disc: Base CSR + CSR+ scene packs + single-disc-on-csr (no manip-movies)

Both stacks need SNOVA (in the core single-disc-on-csr pack).

### Alternative: one fat CSR pack

Merge manip movies into single-disc-on-csr (still must not use that fat pack with CSR+
if avoiding movie payload - then need a thin CSR+ variant). Split remains cleaner.

### Do not

- Put manip movie copies in Clean or Highwind single-disc packs
...
- 2026-08-03-single-disc-full-run-scope.md
- 2026-08-03-single-disc-fmv-wait-vs-stream.md
- 2026-08-04-single-disc-supernova-all-bases.md

# Finding: single-disc-on-csr v0.1.1 build status
...
1. Applied Clean single-disc FIELD trims for every single-disc map **except BLACKBGB**
...
Pack: builder/single-disc-on-csr-v0.1.1/
...
1. Open workspace/iso-extract/ff7_d1_csr_single_disc_work.bin in Makou Reactor
...
4. Rebuild layer vs CSR base (not pristine Clean) with bin_diff_to_layer:
   ...
   - work: workspace/iso-extract/ff7_d1_csr_single_disc_work.bin
   - out: builder/single-disc-on-csr-v0.1.1/layers/disc1.layer.json
   - layer_id: single-disc-on-csr-v0.1.1-disc1

## CSR+ stacking

CSR+ scene packs checked do **not** touch single-disc maps on D1 (aerith-house).
cota/hojo/endgame are disc2/disc3 layers only.

Stack: **csr-v0.14.1** + CSR+ scenes + **single-disc-on-csr-v0.1.1**

...
1. **Search → For Strings...** across the whole program (check "Search
   ...
   defined strings) for a text literal, or **Search → For Scalars** for a
   numeric literal.
2. If found: right-click the hit → **References → Show References to
   Address** (or `Ctrl+Shift+F`). Each xref is a place in code that loads
   it — open the containing function and decompile (`Ctrl+E` in Listing,
   or right-click → **Decompile**).
3. To scope a scalar search to one function instead of the whole program:
   right-click the function in Listing → **Select Function**, then run the
   scalar search restricted to the current selection.
4. Zero hits doesn't mean "not hardcoded" — it can mean the value is

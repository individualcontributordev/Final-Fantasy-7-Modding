...
**Same turn rule:** when a fact is confirmed (including DuckStation/CE playtest and builder verify PASS/FAIL), write or update `docs/findings/` **before** the next unrelated topic — not chat-only, not “end of session later.”
...
User-facing next steps always go **in chat** as **one atomic task** (full GUI/commands, addresses, expected observations). File has COPY-PASTE values + EVIDENCE paste only — **never** yes/no answer blanks. See `.agents/rules/agent-human-workflow.mdc`.
